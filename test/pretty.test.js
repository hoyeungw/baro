...
import { decoSamples, decoString, logger, ros, says, Xr } from '@spare/logger'
...
const BARO_LAYOUT = {
  ...
  format(state) {
    ...
    if (value < total) {
      return `${start} [${ros(agent)}] [${code}] ${bar} | eta ${eta} | ${author} | ${work}`
    }
    else {
      return `${start} [${ros(agent)}] [${code}] ${bar} | ${author} | ${work}`
    }
  },
}
